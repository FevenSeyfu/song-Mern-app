...
export const createSong = async (req, res) => {
  ...
  try {
    ...
    return res.status(201).json({success:true,message:'Song Successfully Added' ,result:song});
  } catch (error) {
...
export const updateSong = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ success:true ,message: "Song Updated successfully",result:song });
  } catch (error) {
    console.log(error.message);
    res.status(500).json({ success: false, message: error.message });
  }
...
export const removeSong = async (req, res) => {
  try {
    ...
    return res.status(200).json({ success:true,message: "Song Deleted Successfully",result:song });
  } catch (error) {
    console.log(error.message);
    res.status(500).json({ success: false, message: error.message });
  }
